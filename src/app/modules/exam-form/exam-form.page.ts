...
import { subjectDetail } from './classwisesubjectdettail'
...
export class ExamFormPage implements OnInit {
    ...
    public years: string[] = ['2023-2024', '2023-2024', '2024-2025']
    rowData: any;
    names: any;
    monthWiseData: any;
    ...
    ngOnInit() {
        ...
        // console.log(this.filterUniqueClasses(studentData))
        //   this.firebaseService
        //       .getAllStudentSession('2024-2025')
        //       .subscribe((sortedStudents: any[]) => {
        //           console.log(sortedStudents);
        //           this.allStudent = sortedStudents
        //           this.allStudent.forEach(item=>{
        //               this.addStudentRecord(item)
        //           })
        //       });
    }

    getStudent(session: string): void {
        this.subscription = this.firebaseService.getAllExamInfo(session).subscribe(items => {
            this.rowData = items;
    ...
    selectYear(value: string): void {
        ...
        this.getStudent(this.selectedYear);
    }
    ...
    addStudentRecord(student: any) {
        let filteredClass = subjectDetail.find(item => item.class === student.class);
        const updatedClass = { ...filteredClass };
        // If a matching class is found, update it with the student name and session
        if (filteredClass) {
            // Create a copy of the object to avoid modifying the original reference
            const updatedClass = { ...filteredClass };

            updatedClass.name = student.StudentName;
            updatedClass.session = '2024-2025';

            console.log('Updated Record:', updatedClass);
            this.firebaseService.pushItems('examInfo', updatedClass)
        } else {
            console.error(`Class ${student.class} not found in subjectDetail`);
        }
        //     let data={
        //       "class": "KG",
        //       "name": "Sanskriti Raut",
    ...
    filterUniqueClasses(students: any) {
        const uniqueClasses: { [key: string]: any } = {};

        students.forEach((student: any) => {
            const studentClass = student.class;
            // Add the student only if the class is not already in the dictionary
            if (!uniqueClasses[studentClass]) {
                uniqueClasses[studentClass] = student;
            }
        });

        // Return the values of the dictionary, which are the unique students
