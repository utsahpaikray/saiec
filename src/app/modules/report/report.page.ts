...
this.firebaseService.getAllstudent().subscribe(items => {
  this.studentMainInfo= items.filter((item:any)=>item['StudentName'] === this.studentName)[0];
})
  }
  public calculateScore(info: { marks: any[]; score: number; total: number; percentage: string; }){
    let writtenScore= 0;
    let oralScore=0;
    let writtenTotal =0;
    let oralTotal= 0
